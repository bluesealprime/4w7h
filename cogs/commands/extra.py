...
class Extra (commands.Cog ):
  ...
  @commands.hybrid_command(name="devinfo", aliases=["devs"], description="Displays information about the bot developers.")
  @blacklist_check()
  @ignore_check()
  ...
  async def devinfo(self, ctx):
      ...
      container.add_item(ui.TextDisplay("# 👨‍💻 Developer Information"))
      container.add_item(ui.Separator())

      # Developer Details
      dev_id = 783953632974471178
      ...
      if dev_user.avatar:
          container.add_item(
              ui.Section(
                  ui.TextDisplay(dev_info),
                  accessory=ui.Thumbnail(dev_user.avatar.url)
              )
          )
      else:
          container.add_item(ui.TextDisplay(dev_info))

      container.add_item(ui.Separator())
      container.add_item(ui.TextDisplay("Special thanks to all contributors and the support team!"))
  ...
  async def ping (self,ctx ):
    ...
    # Unique status based on latency
    if total_response < 100:
        status = "Faster than light! ⚡"
    elif total_response < 200:
        status = "Smooth sailing! 🌊"
    else:
        status = "Bumpy road ahead! ⛈️"
    ...
    content = (
        f"**Latency Information:**\n"
        f"<:icons_pings:1373173701704683540>  **WebSocket Ping:** `{websocket_ping}ms`\n"
        f"<:icons_pings:1373173701704683540>  **Message Latency:** `{message_latency}ms`\n"
        f"<:icons_pings:1373173701704683540>  **Total Response:** `{total_response}ms`"
    )
    ...
    if self.bot.user.avatar:
        ...
        ping_container.add_item(discord.ui.TextDisplay(content))
