...
class NoPrefix(commands.Cog):
            ...
            async with db.execute("SELECT id FROM np") as cursor:
                ...
                if not ids:
                    await ctx.reply_v2(
                        ...
                        content="No users in the no-prefix list."
                    )
                    return
    ...
    async def np_add(self, ctx, user: discord.User):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if result:
                await ctx.reply_v2(
                    title="Error",
                    content=f"**{user}** is already in the No Prefix list."
                )
                return
    ...
    async def np_remove(self, ctx, user: discord.User):
        async with aiosqlite.connect('db/np.db') as db:
            ...
            if not result:
                await ctx.reply_v2(
                    title="Error",
                    content=f"**{user}** is not in the No Prefix list."
                )
                return
    ...
    async def np_status(self, ctx, user: discord.User):
        async with aiosqlite.connect('db/np.db') as db:
            ...
            await ctx.reply_v2(
                title="No Prefix Status",
                content=f"**User**: [{user}](https://discord.com/users/{user.id})\n**User ID**: {user.id}\n\n**Expiry**: {expire_timestamp}"
            )
