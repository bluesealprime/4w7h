...
class Unmute (commands.Cog ):
    ...
    async def unmute (self,ctx,user:discord.Member ):
        if not user.timed_out_until or user.timed_out_until <=discord.utils.utcnow ():
            await ctx.reply_v2(
                title=f"{user.name} is Not Muted!",
                content="**Requested User is not muted in this server.**\n\nClick on the `Add Timeout` button to mute the mentioned user.",
                thumbnail=self.get_user_avatar(user)
            )
            return
        ...
        try:
            ...
            await ctx.reply_v2(
                ...
                content="I can't unmute a user with higher permissions!",
                thumbnail="https://cdn.discordapp.com/emojis/1294218790082711553.png"
            )
            return 
