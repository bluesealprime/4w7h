...
class LockUnlockView (ui.View ):
    ...
    async def unlock (self,interaction:discord.Interaction,button:discord.ui.Button ):
        ...
        await self.ctx.edit_v2(
            message=self.message,
            title=f"Successfully Unlocked {self.channel.name}",
            content=f"**Channel**: {self.channel.mention}\n**Status**: Unlocked\n**Reason:** Unlock request by {self.author}",
            thumbnail="https://i.ibb.co/TxtQNnyH/2400e46b-9674-4142-b2dc-73244e769c3b.png"
        )
...
class Lock (commands.Cog ):
    ...
    async def lock_command (self,ctx,channel:discord.TextChannel =None ):
        ...
        if channel.permissions_for (ctx.guild.default_role ).send_messages is False:
            await ctx.reply_v2(
                title=f"{channel.name} is Already Locked",
                content=f"**Channel**: {channel.mention}\n**Status**: Already Locked",
                thumbnail="https://i.ibb.co/TxtQNnyH/2400e46b-9674-4142-b2dc-73244e769c3b.png"
            )
            return
